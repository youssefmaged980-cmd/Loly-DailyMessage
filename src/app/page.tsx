  ...
  const fetchMessages = async () => {
    try {
      ...
      setCurrentMessage({
        id: 'error',
        date: "",
        message: "حدث خطأ في تحميل الرسالة 😢"
      });
    } finally {
      setLoading(false);
    }
      ...
      <div className="w-full max-w-[650px] mx-auto p-6 md:p-10 flex flex-col gap-10 relative z-10 min-h-screen">
        ...
        <header className="flex flex-col items-center text-center gap-5 mt-4">
          <div className="relative group">
            ...
            <div className="absolute -inset-4 border-2 border-dashed border-rose/40 rounded-full animate-pulse-slow"></div>
            <div className="absolute -inset-2 border border-gold/60 rounded-full rotate-45"></div>
            ...
            <div className="absolute -bottom-2 -right-4 text-3xl animate-bounce drop-shadow-md z-20">🌺</div>
            <div className="absolute top-4 -left-6 text-2xl animate-pulse drop-shadow-md z-20">💮</div>
            <div className="absolute -top-4 right-2 text-xl animate-float-up drop-shadow-md z-20 opacity-70">✨</div>
          </div>
        ...
        >
          ...
          <div className="relative z-10 w-full mb-2 flex flex-col items-center">
            {loading ? (
              <div className="h-8 w-40 bg-rose/20 animate-pulse rounded-full"></div>
            ) : (
          ...
          <div className="text-2xl md:text-3xl leading-[2] text-text-main font-semibold relative z-10 transition-opacity duration-500 px-4 md:px-12 break-words w-full max-w-full">
            {loading ? (
              <span className="animate-pulse">جاري البحث عن رسالة اليوم...</span>
            ) : (
              currentMessage?.message
            )}
          </div>
        ...
        <section className="mt-8 mb-12 animate-fade-in-up" style={{ animationDelay: "0.3s" }}>
          ...
          <div className="flex flex-col gap-4 max-h-[45vh] overflow-y-auto px-4 py-2 scrollbar-timeline">
            ...
            {!loading && messages.filter(msg => msg.id !== currentMessage?.id).length === 0 && (
